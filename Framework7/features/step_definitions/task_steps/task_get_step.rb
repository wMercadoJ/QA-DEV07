...
#@Functional
Then(/^I expect TaskDetails the property (.*) type (.*)$/) do |task_atr, type_task|
  @property_atr = task_atr
  @type_task = type_task

end

#@Functional
And(/^I expect TaskDetails of number test (.*) id task (.*),kind (.*),is story (.*),description (.*), complete (.*), position (.*), created date (.*), updated date (.*)$/) do |number, id_task, kind, story_id, description, complete, position, created_at, updated_at|
 @number=number.to_i
