...
Feature: Get task
  ...
  Scenario Outline: Get a task details from a story
    ...
    Given I have set a connection to pivotal tracker API service
    When I send my GET request to /projects/<project_id>/stories/<story_id>/tasks
    And I expect TaskDetails of number test <i>, id task <id_task>,kind <kind>,is story <story_id>,description <description>, complete <complete>, position <position>, created date <created_at>, updated date <updated_at>
  ...
  @Functional
  Scenario Outline: Validate the properties of task

    Given I have set a connection to pivotal tracker API service
    When I send my GET request to /projects/<project_id>/stories/<story_id>/tasks
    Then I expect TaskDetails the property <properties>, type <type>
